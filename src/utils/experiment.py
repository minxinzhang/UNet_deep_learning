...
def run(config_vars, data_partitions, experiment_name, partition, GPU="2"):
    ...
    for i in range(len(images)):
        ...
        skimage.io.imsave(config_vars["probmap_out_dir"] + filename, (probmap*255).astype(np.uint8))
    ...
    #average_precision = results.groupby("Threshold").mean(numeric_only=True).reset_index()
    #mean_average_precision = average_precision["Precision"].mean()
    #output["MAP"] = mean_average_precision
    average_performance = results.groupby(["Threshold"]).mean(numeric_only = True).reset_index()
    ...
    print(jaccard_index)
    output["F1"] = average_F1_score
    output["Jaccard"] = jaccard_index
