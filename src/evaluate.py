...
def show(ground_truth, prediction, threshold=0.5, image_name="N"):
    ...
    plt.suptitle("True objects:"+str(len(np.unique(ground_truth))))
    plt.show()
    plt.imshow(diff)
    plt.suptitle("Segmentation errors:"+str(len(missed)))
    plt.show()
    plt.imshow(prediction)
    plt.suptitle("Predicted objects:"+str(len(np.unique(prediction))))
    plt.show()
    plt.imshow(IOU)
